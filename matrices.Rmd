...
fromMatrixFunction <- matrix(data = firstRow, nrow = 5, ncol = 10, dimnames = list(c("row1", "row2", "row3", "row4", "row5"),c("C.1", "C.2", "C.3", "C.4", "C.5","C.6", "C.7", "C.8", "C.9", "C.10")))
fromMatrixFunction

fromMatrixFunction["row2","C.4"]

fromRbindFunction <- rbind(firstRow,secondRow)
colnames(fromRbindFunction) <- firstRow
rownames(fromRbindFunction) <- c("A", "B")
fromRbindFunction

fromCbindFunction <- cbind(firstRow,secondRow)
fromCbindFunction
```
...
```{r}
#HomeWork : Basketball free throw
